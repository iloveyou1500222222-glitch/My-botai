...
from pytgcalls.types import AudioPiped, AudioQuality, VideoPiped, VideoQuality
...
async def play_cmd(client: Client, message: Message):
    ...
    try:
        ...
        await call.play(AudioPiped(url), audio_quality=AudioQuality().high)
        await status_msg.edit_text(f"✅ **သီချင်းဖွင့်နေပါပြီရှင်** 🎵\n🎶 `{title}`")
    except Exception as e:
...
async def vplay_cmd(client: Client, message: Message):
    ...
    try:
        ...
        await call.start()
        await call.join_call(chat_id)
        await call.play(VideoPiped(url), video_quality=VideoQuality().high, audio_quality=AudioQuality().high)
        await status_msg.edit_text(f"✅ **Video ဖွင့်နေပါပြီရှင်** 🎬\n🎥 `{title}`")
    except Exception as e:
        await status_msg.edit_text(f"❌ အမှား: `{e}`")
...
async def stop_cmd(client: Client, message: Message):
    try:
        await call.stop()
        await message.reply_text("⏹️ ရပ်လိုက်ပါပြီရှင် 🥰")
    except Exception as e:
        await message.reply_text(f"❌ ရပ်လို့မရဘူးရှင် 😅\n`{e}`")
...
async def leavevc_cmd(client: Client, message: Message):
    try:
        ...
        await message.reply_text("👋 ထွက်သွားပါပြီရှင် နောက်မှပြန်ဆုံကြမယ်နော် 😘")
    except Exception as e:
        await message.reply_text(f"❌ ထွက်လို့မရဘူးရှင် 😅\n`{e}`")
...
async def callback_handler(client: Client, callback_query: CallbackQuery):
    ...
    if data == "show_lang":
        ...
        help_text = (
            ...
            "• /play သီချင်းနာမည် - သီချင်းဖွင့်မယ်\n"
            "• /vplay ဇာတ်ကားနာမည် - Video ဖွင့်မယ်\n"
            "• /stop - ရပ်မယ်\n"
            "• /leavevc - ထွက်မယ်\n\n"
